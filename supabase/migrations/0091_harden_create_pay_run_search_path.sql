-- 0091: pin create_pay_run's search_path (advisor: function_search_path_mutable).
--
-- Same signature + behaviour as 0085; the only changes are SET search_path = ''
-- and schema-qualifying every table reference (public.*) so the function can't be
-- influenced by the caller's search_path. Built-ins (to_char/lpad/regexp_replace/
-- coalesce/max) still resolve from pg_catalog. SECURITY INVOKER is unchanged, so
-- the caller's own RLS still governs every write.
