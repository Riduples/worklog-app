-- 0083 (Phase D1): payroll statutory fields + auto-numbering.
--
-- The staff register captured enough to RUN payroll but not enough to FILE it.
-- Adds the optional SARS e@syFile/IRP5 + UIF UI-19 fields (DOB, address, bank),
-- an auto employee number (EMP-YYYY-NNNN), and a sequential payslip number
-- (PS-YYYY-NNNN) generated server-side inside create_pay_run so it is atomic and
-- race-safe. All additive — existing rows keep working unchanged.
...
ALTER TABLE pay_runs
  ADD COLUMN payslip_number text;

-- One EMP number per employee and one PS number per payslip, within a business.
-- Partial so the many existing null rows don't collide.
CREATE UNIQUE INDEX uniq_staff_employee_number
  ON staff_register(business_id, employee_number) WHERE employee_number IS NOT NULL;
CREATE UNIQUE INDEX uniq_pay_run_payslip_number
  ON pay_runs(business_id, payslip_number) WHERE payslip_number IS NOT NULL;

-- create_pay_run now also assigns the payslip number. Same signature as before
-- (the client call is unchanged) — the number is derived here, not passed in, so
-- concurrent runs can't diverge from client state and the sequence stays server-
-- authoritative. Everything else (the wages/UIF/SDL expenses, loan repayment and
-- leave records) is byte-for-byte the previous body.
...
BEGIN
  ...
  -- Next payslip number for this business + year (PS-YYYY-NNNN).
  ...
  INSERT INTO pay_runs (
    ...
    status, approved_by, approved_at, payslip_number
  ) VALUES (
