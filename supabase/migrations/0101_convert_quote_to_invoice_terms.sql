-- 0101: carry a Terms & Conditions value through quote→invoice conversion.
-- Adds p_terms (defaults NULL) so a converted invoice can be stamped with the
-- owner's default invoice terms. Drops the old 9-arg signature first so the
-- 10-arg version isn't an ambiguous overload for existing 9-named-arg callers.
DROP FUNCTION IF EXISTS public.convert_quote_to_invoice(uuid, text, jsonb, numeric, numeric, numeric, numeric, date, date);
...
BEGIN
  ...
  INSERT INTO invoices (
    ...
    status, converted_from_quote_id, vat_rate, vat_amount, terms
  ) VALUES (
    ...
    p_issue_date, p_due_date, 'unpaid', v_quote.id, p_vat_rate, p_vat_amount, p_terms
  )
