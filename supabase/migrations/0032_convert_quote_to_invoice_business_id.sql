-- The RPC inserts an invoices row directly, bypassing the hook-layer insert
-- that now supplies business_id everywhere else. Propagate it from the
-- source quote row (which already carries it) so invoices.business_id NOT
-- NULL isn't violated when converting a quote.
