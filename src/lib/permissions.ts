...
export const TOOL_CATEGORIES: { id: string; label: string; icon: string; desc: string; tools: ToolId[] }[] = [
  ...
  { id: "bookings", label: "Scheduling System", icon: "📅", desc: "Your appointment diary, hours worked & business trips — plan how you spend your day", tools: ["booking", "timetrack", "mileage"] },
...
export const TOOL_LABELS: Partial<Record<ToolId, { icon: string; label: string; desc?: string }>> = {
  ...
  booking: { icon: "📓", label: "Diary", desc: "Books appointments with customers and suppliers" },
  ...
  timetrack: { icon: "⏱️", label: "Time Log", desc: "Log hours per customer — links to quotes, rates & appointments" },
