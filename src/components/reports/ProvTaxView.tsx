...
export function ProvTaxView() {
  ...
  const ytdIncome = (income ?? []).reduce((s, r) => s + Number(r.amount), 0);
  ...
  const netTax =
    entityType === "sole_prop" ? Math.max(0, grossTax - taxRates.PRIMARY_REBATE - medCredit) : Math.max(0, grossTax);
  ...
  const DarkRow = ({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) => (
  ...
  return (
    <div style={{ padding: "20px 16px 100px" }}>
      ...
      <div style={{ background: "#0C4A6E", borderRadius: 14, padding: "16px 18px", marginTop: 8, marginBottom: 10 }}>
        ...
        {entityType === "sole_prop" && <DarkRow label="Less: primary rebate" value={`−${fmt(taxRates.PRIMARY_REBATE)}`} />}
