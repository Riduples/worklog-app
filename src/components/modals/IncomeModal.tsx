...
export function IncomeModal({
  ...
  // Linked to an invoice? Then that document is the record and carries the
  // category, so the "what for" step is skipped — the same rule the expense
  // side follows.
  ...
  const isMatched = !isOther && !!matchedInvoiceId;
    ...
    <Modal title={isEdit ? "Edit money in" : isOther ? "Money in" : "Log income"} onClose={onClose}>
      ...
      <InvoiceMatcher
        ...
        onMatch={(id) => {
          ...
          setMarkPaid(!!id);
          // Linking an invoice makes it the record for this money, so clear any
          // half-started category — "what for" is only for unmatched cash sales.
          if (id) {
            setWhatFor("");
